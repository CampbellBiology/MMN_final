...
public class joinServlet extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try {
         ...
         response.sendRedirect(context+"/resources/view_0427/Login2.html");
         }
         
      } catch (Exception e) {
         response.setContentType("text/html; charset=UTF-8");
         PrintWriter writer = response.getWriter();
         writer.println("<script>alert('회원가입 에러발생'); location.href='"+context+"/resources/view_0427/Login2.html"+"';</script>"); 
         writer.close();
         e.printStackTrace();
      }
   }
