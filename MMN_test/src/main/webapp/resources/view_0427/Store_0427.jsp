...
<body>
	...
	<main>
		...
		: "https://raw.githubusercontent.com/CampbellBiology/MMN2/master/MMN_test/src/main/webapp/resources/UI/UI/keep_btn.png"%>"
			...
			<!-- 여기서부터 리뷰작성란 -->
			<form method="post" id="review_form" action="review" enctype="multipart/form-data">
				<div id="create_riview">
					...
					<input type="file" id="reviewimg" name="asdf" accept=".jpg, .png"
		...
		<script>
         ...
         function select_menu() {
            ...
            if (Menu_data != "" && menu_count < 3 && menu_dup) {
               ...
               document.getElementById("menuListView").value = menu_list.toString(); //DB 저장용, 보이지는 않음
            ...
            } else if (menu_count > 3 && menu_dup) { //선택한 메뉴 갯수가 4개 이상이라면 숨겼던 div보여주고 text변경. count가 4개 이상이라면 그 외 *개 보이게
               //리스트에 추가
               menu_list.push(Menu_data);
               menu_count = menu_list.length;
               //text수정
               ...
               console.log("none");  //입력이나 처리 없음
            }
            menu_count = menu_list.length;  //길이따라 count 재지정.
         }
